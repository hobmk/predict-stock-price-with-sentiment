...
def add_features(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    ...
    # 변동성: 20일 표준편차(비모수)
    df["sigma20"] = (
        g["ret1"].rolling(20, min_periods=10).std().reset_index(level=0, drop=True)
    )
...
def fit_scaler(train: pd.DataFrame, feat_cols: list, by_ticker=True):
    ...
    if by_ticker:
        for tkr, g in train.groupby("ticker"):
            ...
            stats[tkr] = {
                "mean": mu,
                "std": {k: float(sd[k]) if pd.notnull(sd[k]) else 1.0 for k in sd},
            }
    else:
...
def save_panel(df: pd.DataFrame, path: str, feat_cols: list):
    panel = df.rename(columns={"ticker": "unique_id", "date": "ds"})[
        ["unique_id", "ds", "y", *feat_cols]
    ]
...
def make_windows(df: pd.DataFrame, seq_len: int, feat_cols: list):
    ...
    Xs = (
        np.stack(Xs) if Xs else np.zeros((0, seq_len, len(feat_cols)), dtype=np.float32)
    )
...
def main():
    ...
    ap.add_argument(
        "--by-ticker-scaling", action="store_true", help="티커별 표준화(권장)"
    )
    args = ap.parse_args()
